...
class Http(object):
    ...
    def _request(self, sock, method, path, protocol_version, headers, data, bufsize=__bufsize__, crlf=0):
    ...
    def request(self, method, url, data=None, headers={}, fullurl=False, bufsize=__bufsize__, crlf=0):
...
class Common(object):
    ...
    def __init__(self):
        ...
        self.AUTORANGE_SPAWNDELAY = self.CONFIG.getfloat('autorange', 'spawndelay')
...
class RangeFetch(object):
    ...
    def __init__(self, sock, response_code, response_headers, response_rfile, method, url, headers, payload, fetchservers, password, maxsize=0, bufsize=0, waitsize=0, threads=0, spawndelay=0):
    ...
    if need_direct:
        ...
        try:
            ...
            if code == 206:
                ...
                rangefetch = RangeFetch(sock, code, response_headers, response_rfile, method, path, headers, request_payload, fetchservers, common.GAE_PASSWORD, maxsize=common.AUTORANGE_MAXSIZE, bufsize=common.AUTORANGE_BUFSIZE, waitsize=common.AUTORANGE_WAITSIZE, threads=common.AUTORANGE_THREADS, spawndelay=common.AUTORANGE_SPAWNDELAY)
...
def main():
    ...
    if common.LISTEN_DEBUGINFO > 0:
        logging.basicConfig(level=logging.DEBUG if common.LISTEN_DEBUGINFO==2 else logging.INFO, format='%(levelname)s - %(asctime)s %(message)s', datefmt='[%b %d %H:%M:%S]')
    else:
